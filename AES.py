Sbox = [

]

InvSbox=[0]*256
for i in range(256):
    InvSbox[Sbox[i]]=i

Rcon=[0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36]

def xtime(a):
    a<<=1
    if a&0x100:
        a^=0x11b
    return a&0xff

def mul(a,b):
    p=0
    for _ in range(8):
        if b&1:
            p^=a
        a=xtime(a)
        b>>=1
    return p&0xff

def key_expansion(key):
    Nk,Nr=4,10
    w=[list(key[4*i:4*i+4]) for i in range(Nk)]
    for i in range(Nk,4*(Nr+1)):
        temp=list(w[i-1])
        if i%Nk==0:
            temp=temp[1:]+temp[:1]
            temp=[Sbox[x] for x in temp]
            temp[0]^=Rcon[i//Nk-1]
        w.append([w[i-Nk][j]^temp[j] for j in range(4)])
    round_keys=[]
    for r in range(Nr+1):
        rk=[]
        for c in range(4):
            rk+=w[4*r+c]
        round_keys.append(rk)
    return round_keys

def add_round_key(state,rk):
    for i in range(16):
        state[i]^=rk[i]

def sub_bytes(state,inv=False):
    box=InvSbox if inv else Sbox
    for i in range(16):
        state[i]=box[state[i]]

def shift_rows(state,inv=False):
    s=state[:]
    for r in range(1,4):
        row=[s[r+4*c] for c in range(4)]
        row=row[-r:]+row[:-r] if inv else row[r:]+row[:r]
        for c in range(4):
            state[r+4*c]=row[c]

def mix_columns(state,inv=False):
    for c in range(4):
        col=state[4*c:4*c+4]
        if not inv:
            state[4*c+0]=mul(col[0],2)^mul(col[1],3)^col[2]^col[3]
            state[4*c+1]=col[0]^mul(col[1],2)^mul(col[2],3)^col[3]
            state[4*c+2]=col[0]^col[1]^mul(col[2],2)^mul(col[3],3)
            state[4*c+3]=mul(col[0],3)^col[1]^col[2]^mul(col[3],2)
        else:
            state[4*c+0]=mul(col[0],14)^mul(col[1],11)^mul(col[2],13)^mul(col[3],9)
            state[4*c+1]=mul(col[0],9)^mul(col[1],14)^mul(col[2],11)^mul(col[3],13)
            state[4*c+2]=mul(col[0],13)^mul(col[1],9)^mul(col[2],14)^mul(col[3],11)
            state[4*c+3]=mul(col[0],11)^mul(col[1],13)^mul(col[2],9)^mul(col[3],14)

def encrypt_block(block,rk):
    state=list(block)
    add_round_key(state,rk[0])
    for r in range(1,10):
        sub_bytes(state); shift_rows(state); mix_columns(state); add_round_key(state,rk[r])
    sub_bytes(state); shift_rows(state); add_round_key(state,rk[10])
    return bytes(state)

def decrypt_block(block,rk):
    state=list(block)
    add_round_key(state,rk[10])
    for r in range(9,0,-1):
        shift_rows(state,True); sub_bytes(state,True); add_round_key(state,rk[r]); mix_columns(state,True)
    shift_rows(state,True); sub_bytes(state,True); add_round_key(state,rk[0])
    return bytes(state)

def pad(data):
    p=16-len(data)%16
    return data+bytes([p]*p)

def unpad(data):
    return data[:-data[-1]]

def aes_encrypt(text,key):
    key=key.ljust(16,b'\0')[:16]
    rk=key_expansion(key)
    data=pad(text.encode())
    out=b''
    for i in range(0,len(data),16):
        out+=encrypt_block(data[i:i+16],rk)
    return out

def aes_decrypt(cipher,key):
    key=key.ljust(16,b'\0')[:16]
    rk=key_expansion(key)
    out=b''
    for i in range(0,len(cipher),16):
        out+=decrypt_block(cipher[i:i+16],rk)
    return unpad(out).decode()

if __name__=="__main__":
    mode=input("Mode (encrypt/decrypt): ").strip().lower()
    key=input("Enter key: ").encode()
    if mode=="encrypt":
        text=input("Enter plaintext: ")
        print("Cipher (hex):",aes_encrypt(text,key).hex())
    elif mode=="decrypt":
        cipher=bytes.fromhex(input("Enter ciphertext (hex): "))
        print("Plaintext:",aes_decrypt(cipher,key))
    else:
        print("Invalid mode")
